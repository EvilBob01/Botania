...
public class ElvenTradeProcessor implements IComponentProcessor {
	...
	public IVariable process(Level level, String key) {
		...
		if (key.equals("heading")) {
			return IVariable.wrap(recipes.get(0).getOutputs().get(0).getHoverName().getString());
	...
	private IVariable interweaveIngredients(int inputIndex, Level level) {
		...
		return PatchouliUtils.interweaveIngredients(recipes, longestIngredientSize, level.registryAccess());
	}
