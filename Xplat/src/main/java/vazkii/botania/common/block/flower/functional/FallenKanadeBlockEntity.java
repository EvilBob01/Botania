...
public class FallenKanadeBlockEntity extends FunctionalFlowerBlockEntity {
	...
	public void tickFlower() {
		...
		if (!getLevel().isClientSide) {
			boolean did = false;
			List<LivingEntity> entities = getLevel().getEntitiesOfClass(LivingEntity.class, new AABB(getEffectivePos().getX() - RANGE, getEffectivePos().getY() - RANGE, getEffectivePos().getZ() - RANGE, getEffectivePos().getX() + RANGE + 2.0, getEffectivePos().getY() + RANGE + 2.0, getEffectivePos().getZ() + RANGE + 2.0), FallenKanadeBlockEntity::canHeal);
