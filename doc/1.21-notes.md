### Completed porting tasks (NeoForge 1.21.1 branch, as of 2026-06)

* ✅ Java 17 → Java 21 (build toolchain updated)
* ✅ Build system: ForgeGradle → NeoForge ModDev Gradle 2.0 plugin (`net.neoforged.moddev` 2.0.78)
* ✅ Gradle wrapper bumped to 8.10
* ✅ All `net.minecraftforge.*` imports → `net.neoforged.*` (37 Forge-side files)
* ✅ `Block.Properties.copy()` → `ofFullCopy()` (~424 call sites)
* ✅ `FrameType`, `AttributeModifier.Operation`, `BlockSource` accessors renamed
* ✅ `vertex()` → `vertices()` in geometry rendering
* ✅ Recipe API: `CraftingContainer` → `CraftingInput`, `RegistryAccess` → `HolderLookup.Provider`; created `BotaniaContainer implements RecipeInput` wrapper for custom recipe types
* ✅ Model rendering: `renderToBuffer(PoseStack,VertexConsumer,int,int,float,float,float,float)` → `(PoseStack,VertexConsumer,int,int,int)` packed ARGB color
* ✅ `com.mojang.blaze3d.vertex.ARGB` removed → `net.minecraft.util.FastColor.ARGB32`
* ✅ `DefaultVertexFormat.POSITION_COLOR_TEX` removed → `POSITION_TEX_COLOR`; `getPositionColorTexShader()` → `getPositionTexColorShader()`; `POSITION_COLOR_TEX_SHADER` render state shard replaced
* ✅ `halo.json` shader updated (vertex program name + attribute order)
* ✅ Particle API: `Deserializer<T>` removed, `writeToNetwork`/`writeToString` removed; `codec()` returns `MapCodec<T>`; added `streamCodec()` (`WispParticleData`, `SparkleParticleData`)
* ✅ `SavedData.Factory` pattern (`SkyblockSavedData`)
* ✅ `GaiaGuardianEntity`: `getDefaultLootTable()` → `ResourceKey<LootTable>`, `finalizeSpawn` signature, `canChangeDimensions(ServerLevel,ServerLevel)`, `canBeLeashed()`, `getAddEntityPacket(ServerEntity)`, `EquipmentSlot.Type.HUMANOID_ARMOR`
* ✅ `BlockSource` accessor renames: `.getPos()` → `.pos()`, `.getLevel()` → `.level()`, `.getBlockState()` → `.state()` (11 dispenser files)
* ✅ `AbstractProjectileDispenseBehavior` → `ProjectileDispenseBehavior`
* ✅ `MobEffect.applyEffectTick` return type `void` → `boolean`
* ✅ `DataResult.get().orThrow()` → `DataResult.getOrThrow()`
* ✅ Advancements: `conditionsFromItem` returns `Criterion<?>` not `TriggerInstance`

### Remaining porting tasks (NeoForge 1.21.1, still TODO)

* ⬜ Loot provider API: `Deserializers.createLootTableSerializer()` → `LootTable.DIRECT_CODEC.encodeStart(...)`, `SetNbtFunction` → `SetCustomDataFunction`, `CopyNbtFunction` → `CopyCustomDataFunction`
* ⬜ ItemStack NBT API fully removed: `getOrCreateTag()`, `getTag()`, `hasTag()`, `ItemStack.of(CompoundTag)`, `removeTagKey()` → DataComponents API across many files
* ⬜ `hasCustomHoverName()` → `stack.has(DataComponents.CUSTOM_NAME)`
* ⬜ `Item.Properties.defaultDurability(int)` → `durability(int)`
* ⬜ `Blocks.GRASS` → `Blocks.SHORT_GRASS`
* ⬜ `Holder<Attribute>` ↔ `Attribute` and `MobEffect` → `Holder<MobEffect>` mismatches
* ⬜ `ResourceKey<Enchantment>` → `Holder<Enchantment>`; enchantment helper API changes
* ⬜ `PotionUtils` removed → `PotionContents`
* ⬜ `EnchantmentHelper.getEnchantments(ItemStack)` → `ItemEnchantments`
* ⬜ AssemblyHaloItem, ManaEnchanterBlockEntity, RosaArcanaBlockEntity, JEIBotaniaPlugin remaining errors
* ⬜ Full compile success, then runtime testing
